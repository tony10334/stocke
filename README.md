...
右上角齒輪是「觀察設定」：比較窗口（隔幾個資料日比）、統計口徑（算 ETF 檔數或投信家數）、門檻、方向（加碼／減碼／兩者）、規模調整後強度（每單位變動門檻 %），以及各區塊與圖表的顯示開關。共同訊號會在瀏覽器端依設定即時重算（用 `site-data.json` 的 `holdings_history`），設定存在瀏覽器 localStorage，不影響資料。

歷史可回補：統一投信的 PCF 可查到上市日，富邦從 2026-06-09 上市日起，野村、國泰、摩根都能查到更早。`data/` 目前從 2026-06-01 起（統一的現金明細只有最新一天，歷史日的現金水位以非股票水位呈現）。

共同訊號門檻預設 3 檔（`etfs.json` 的 `co_signal_min_etfs`），觀察設定裡可臨時改。
...
fetchers/
  ...
  uni.py             統一投信：GetPCF API + 基金頁內嵌 DataAsset（00981A、00403A）
  fubon.py           富邦投信：Assets.aspx 伺服器端渲染的 HTML 表格（00405A）
  nomura.py          野村投信：GetFundAssets JSON API（00980A、00985A、00999A）
  cathay.py          國泰投信：cwapi 四個 GET 端點（00400A）
  jpm.py             摩根投信：申購買回清單 Excel，含持股四張工作表（00401A）
  xlsx.py            標準函式庫的 .xlsx 讀取器（給摩根用）
...
| 代號 | 名稱 | 投信 | 來源 | 歷史回補 | 現金明細 |
|---|---|---|---|---|---|
| 00981A | 主動統一台股增長 | 統一 | GetPCF API + 基金頁 DataAsset | 可 | 只有最新一天 |
| 00403A | 主動統一升級50 | 統一 | 同上 | 可 | 只有最新一天 |
| 00405A | 主動富邦台灣龍耀 | 富邦 | Assets.aspx | 可 | 有 |
| 00980A | 主動野村臺灣優選 | 野村 | GetFundAssets API | 可 | 有 |
| 00985A | 主動野村台灣50 | 野村 | 同上 | 可 | 有 |
| 00999A | 主動野村臺灣高息 | 野村 | 同上 | 可 | 有 |
| 00400A | 主動國泰動能高息 | 國泰 | cwapi | 可（至少到 4 月） | 有 |
| 00401A | 主動摩根台灣鑫收 | 摩根 | m12_pcf Excel | 可 | 有（現金與約當現金） |

各家都在營業日 T 的 16:30 後或稍晚揭露 **T 日**持股：

- 統一：PCF 公告日標為 T+1 營業日，基金頁的現金明細同時更新為 T 日（且只保留最新一天，回補不到）。
- 富邦：`Assets.aspx?ddate=T` 在 16:30 後就回 T 日資料。
- 野村：`SearchDate=T` 直接查 T 日。
- 國泰：`SearchDate=T`，回傳的 `preDate` 才是資料日。
- 摩根：PCF Excel 的公告日是 T+1 營業日，實際上線時間可能到隔天；fetcher 會往後試幾個公告日，`fetch.py` 也會在單日模式補抓前 3 個營業日缺漏的快照（`--catch-up`）。

野村與國泰的持股沒有金額欄，快照的 `amount` 以「權重 × 淨資產」推算（`extra.amount_basis`）。國泰與摩根有台指選擇權部位（掩護性買權），放在 `extra.options`，不算進股票也不算進非股票明細。

所以 17:30 跑 `python fetch.py` 抓到的是**當天**持股；統一的現金明細錯過就沒了，這是排程不能漏跑、21:00 要補抓的原因。
